...
for cur_dir, dirs, files in os.walk(DATA_DIR + "/Rarity"):
    for file in files:
        with open(cur_dir + "/" + file) as f:
            rarity_json = json.load(f)
            limits = rarity_json["dl"]
            rarity_limits[file[1:-5]] = [limits["vis"], limits["inv"]]
        ...
        with open(cur_dir + "/" + file) as f:
            ...
            if dna_json["$type"].startswith("CrDNARe"):
                if dna_json["l"]:
                    ...
                    limit, hard_limit = rarity_limits[rarity]
                    id = dna_json["TypeId"]
